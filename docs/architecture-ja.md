...
flowchart TB
    Browser[ブラウザ]
    subgraph WorkOS["work-os プロセス（Docker またはローカル）"]
        Next["Next.js 16 (App Router)<br/>src/app/api/** REST エンドポイント"]
        Express["Express 5 (src/server.ts)<br/>Socket.IO 4 — WS サーバー<br/>/healthz — ヘルスチェック"]
        Pool["MultiHostSessionPool"]
        LocalP["LocalTmuxProvider (ソケット)"]
    ...
    HostTmux["ホスト tmux<br/>/tmp/tmux-1000/default"]
    Remote["リモートエージェント<br/>:3001/api/sessions"]
    Browser -- "HTTP (Next.js ページ + API ルート)" --> Next
    Browser -- "WebSocket (Socket.IO — ターミナルストリーム)" --> Express
    LocalP -- "ソケット bind-mount" --> HostTmux
    SshP -- "SSH" --> HostTmux
