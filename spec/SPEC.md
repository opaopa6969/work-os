...
```mermaid
graph TB
    Browser["Browser<br/>(xterm.js + React)"]
    Express["Express 5 + socket.io Server<br/>(Node.js single process)"]
    NJ["Next.js 16 App Router"]
    Tmux["tmux sessions<br/>(local / SSH / WSL)"]
    CF["Cloudflare Tunnel<br/>(外部公開)"]

    Browser <-->|"WebSocket (socket.io)"| Express
    Browser <-->|HTTP| NJ
    Express --- NJ
    Express <-->|"PTY (node-pty) / SSH / HTTP Agent"| Tmux
    CF -->|":3000"| Express
```
...
graph TB
    ...
    subgraph LocalHost["Local Host"]
        PTY1["PtyBridge<br/>node-pty"]
        MIR1["MirrorBridge<br/>capture-pane"]
        TMUX1["tmux sessions<br/>local socket"]
        PTY1 --> TMUX1
        MIR1 --> TMUX1
    end

    subgraph SSHHost["SSH Remote Host"]
        PTY2["PtyBridge<br/>ssh -t tmux"]
        MIR2["MirrorBridge<br/>ssh tmux capture-pane"]
        TMUX2["tmux sessions<br/>remote socket"]
        PTY2 --> TMUX2
        MIR2 --> TMUX2
    end

    subgraph HTTPAgent["HTTP Agent Host"]
        RWSB["RemoteWebSocketBridge<br/>socket.io-client"]
        AGT["Work OS Agent<br/>npm run dev:agent"]
        RWSB -->|WebSocket| AGT
    end
...
stateDiagram-v2
    direction TB
    [*] --> idle
    idle --> connecting : socket.connect 発火
    connecting --> connecting : start イベント送信後
    connecting --> ready : terminal:status { state: 'ready' } 受信
    connecting --> error : terminal:error 受信 / connect_error
...
```mermaid
flowchart TD
    A["[socket: start イベント受信]"]
    B["resolve(compositeId)<br/>→ provider + sessionName"]
    C{HttpRemoteProvider?}
    D["ensureRemoteWebSocketBridge()<br/>→ RemoteWebSocketBridge"]
    E["getSessionInfo() → mode判定"]
    F{mode === 'mirror'?}
    G["ensureMirrorBridge()<br/>→ setInterval 400ms (capture-pane)<br/>→ terminal:snapshot ブロードキャスト"]
    H["ensurePtyBridge()<br/>→ node-pty spawn (tmux attach-session / ssh)<br/>→ onData → output ブロードキャスト<br/>→ onExit → session-exit ブロードキャスト → bridge 削除"]
    ...
    Z["[socket disconnect]"]
    ZA["detachSocket(socket.id)<br/>→ bridge.sockets から削除"]
    ZB{sockets.size === 0?}
    ZC["mirror: clearInterval + bridges.delete"]
    ZD["remote-websocket: remoteSocket.disconnect + bridges.delete"]
    ZE["pty: ブリッジは残存<br/>(他ソケットが接続中の可能性)"]

    Z --> ZA --> ZB
    ZB -->|mirror| ZC
    ZB -->|remote-websocket| ZD
    ZB -->|pty| ZE
```
...
```mermaid
flowchart TD
    Start["getSessionInfo(provider, sessionName, preferredMode)"]
    Gather["attachedCount = tmux display-message #session_attached<br/>currentCommand = #pane_current_command<br/>isShellCommand = command in bash/sh/zsh/fish<br/>isChildShell = sessionId.startsWith('sh-')"]
    DetectCond{"attachedCount > 0<br/>OR (not isShellCommand AND not isChildShell)?"}
    DetectedMirror["detectedMode = 'mirror'"]
    DetectedPty["detectedMode = 'pty'"]
    FinalCond1{"preferredMode in<br/>mirror, readonly-mirror?"}
    FinalCond2{"preferredMode in<br/>attach, resize-client?"}
    FinalMirror["finalMode = 'mirror'"]
    FinalPty["finalMode = 'pty'"]
    FinalDetected["finalMode = detectedMode"]

    Start --> Gather --> DetectCond
    DetectCond -->|YES| DetectedMirror
    DetectCond -->|NO| DetectedPty
    DetectedMirror --> FinalCond1
    DetectedPty --> FinalCond1
    FinalCond1 -->|YES| FinalMirror
    FinalCond1 -->|NO| FinalCond2
    FinalCond2 -->|YES| FinalPty
    FinalCond2 -->|NO| FinalDetected
```
...
```mermaid
flowchart TD
    Start["start(commanderSessionId, targetSessionId, pool)"]
    Timer["setInterval(5000, pollAndRespond)"]
    Poll["capture-pane → content"]
...
sequenceDiagram
    ...
    UI->>API: POST /api/sessions<br/>{ sessionRole: 'commander', linkedSessionId }
    ...
    loop every 5 seconds
        ...
        alt isWaitingForInput == true
            AAM->>AAM: selectKey(content)<br/>→ '1\n' or 'y\n'
...
sequenceDiagram
    ...
    Server-->>Browser: terminal:status { state:'connected' }
    Browser->>Server: start { sessionId, cols, rows, mode }
    Server->>Server: resolve(compositeId)
    Server->>Tmux: getSessionInfo()
    Server->>Tmux: node-pty.spawn (tmux attach-session)
    Server-->>Browser: terminal:status { state:'ready' }
    Tmux-->>Server: pty onData
    Server-->>Browser: output (raw)
    Browser->>Server: command { data }
    Server->>Tmux: pty.write(data)
    Browser->>Server: resize { cols, rows }
    Server->>Tmux: pty.resize() [+ tmux resize-window]
    Browser->>Server: disconnect
    Server->>Server: detachSocket() (pty bridge 残存)
```
...
sequenceDiagram
    ...
    Browser->>Server: socket.io connect
    Server-->>Browser: terminal:status { state:'connected' }
    Browser->>Server: start { sessionId, mode:'mirror' }
    Server->>Server: getSessionInfo()
    Server->>Server: ensureMirrorBridge()
    Server-->>Browser: terminal:status { state:'ready' }
    Server-->>Browser: terminal:snapshot (初回フルスナップショット)
    loop 400ms ごと
        Server->>Tmux: capture-pane
        Tmux-->>Server: output
        alt 変化あり
            Server-->>Browser: terminal:snapshot (broadcast)
        else 変化なし
            Server->>Server: skip
        end
    end
    Browser->>Server: command { data }
    Server->>Server: sendMirrorData()
    Server->>Tmux: send-keys -l (バッファリング送信)
    Browser->>Server: disconnect
    Server->>Server: detachSocket()
    Server->>Server: clearInterval()
    Server->>Server: bridges.delete()
```
...
sequenceDiagram
    ...
    participant RemoteAgent as Remote Agent (HTTP)

    Browser->>LocalServer: start
    LocalServer->>LocalServer: ensureRemoteWS()
    LocalServer->>RemoteAgent: ioClient connect
    RemoteAgent-->>LocalServer: connect
    LocalServer->>RemoteAgent: emit 'start'
    LocalServer-->>Browser: terminal:status { state:'ready' }
    RemoteAgent-->>LocalServer: output
    LocalServer-->>Browser: output
    Browser->>LocalServer: command
    LocalServer->>RemoteAgent: emit 'command'
    Browser->>LocalServer: resize
    LocalServer->>RemoteAgent: emit 'resize'
    Browser->>LocalServer: disconnect
    LocalServer->>RemoteAgent: remoteSocket.disconnect()
    LocalServer->>LocalServer: bridges.delete()
```
